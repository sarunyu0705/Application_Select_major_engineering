  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('ล็อคอิน'),
        backgroundColor: Colors.red,
        automaticallyImplyLeading: false, // Close ปุ่มBackForward NajaEiEi
      ),
...
// ############################# Set Futrue กันใหม่โว้ยยย ####################//
  Future login_type1({String? std_id, String? std_password}) async {
    String? api_std_id =
  ...
  Future login_type2({String? std_idcard, String? std_password}) async {
    ...
    await Dio().get(api_std_idcard).then((value) async {
      print('#### value for Api ==>> $value');
      if (value.toString() == 'null') {
        ScaffoldMessenger.of(context).showSnackBar(snackBar_idcard_false);
      } else {
        for (var item in json.decode(value.data)) {
          MyUserModel model = MyUserModel.fromMap(item); //**//
          if (std_password == model.std_password) {
